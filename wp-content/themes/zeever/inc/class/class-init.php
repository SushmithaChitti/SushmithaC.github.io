...
class Init {
	...
	public function notice_install_plugin() {
		...
		if ( $is_gutenverse_installed ) {
			...
			$button_text = __( 'Activate Gutenverse Plugin', 'zeever' );
			...
			$button_text = __( 'Install Gutenverse Plugin', 'zeever' );
		...
		<style>
			.install-gutenverse-plugin-notice {
				...
				position: relative;
				overflow: hidden;
				background-image: url(<?php echo esc_url( ZEEVER_URI . '/assets/images/mockup-2x.webp' ); ?>);
	...
	public function load_zeever_dashboard() {
		...
		if ( $is_gutenverse_installed && $gutenverse_active ) {
			...
			$button_text = __( 'Install Gutenverse Version', 'zeever' );
			$button_link = wp_nonce_url( self_admin_url( 'themes.php?page=zeever-dashboard&install-template=gutenverse' ), 'install-template_gutenverse' );
		...
		?>
			...
			<div class="install-template">
				<div class="thumbnail">
					<img src="<?php echo esc_html( ZEEVER_URI . '/screenshot.jpg' ); ?>" alt="Zeever" width="400" height="300"/>
				</div>
				<div class="content">
					<h1><?php echo esc_html_e( 'Zeever Template (Gutenverse version)', 'zeever' ); ?></h1>
					<span><?php echo esc_html_e( 'To get the best experience using Zeever theme, you need to install and activate Gutenverse Plugin. With Gutenverse plugin installed, you gain access to Zeever advance template and block patterns for free which all built using Gutenverse blocks. Also please backup your current templates if you have any changes to it, installing new template might overwrite or conflict with the current changes.', 'zeever' ); ?></span>
					<div>
						<div class="gutenverse-bottom">
							...
							<?php } ?>
							<a class="gutenverse-link" href="<?php echo esc_url( 'https:/gutenverse.com/demo?name=zeever' ); ?>" target='_blank'>
								<?php echo esc_html_e( 'View Live Demo →', 'zeever' ); ?>
							</a>
						</div>
					</div>
				</div>
